...
(function() {
    ...
    function buildBannerHTML(context) {
        ...
        if (isActive) {
            return `<div class="favor-banner-active" style="${isCompact?'padding:5px 10px;':'padding:6px 14px;'}background:var(--accent-bg);border-top:1px solid var(--accent-glow);border-bottom:1px solid var(--accent-glow);display:flex;align-items:center;justify-content:space-between;gap:8px;cursor:pointer;width:100%;box-sizing:border-box;height:30px;" onclick="window.showQuestHub && window.showQuestHub(); window._questHubTab = 'favor'; setTimeout(function(){ if(window._renderQuestHubTab) window._renderQuestHubTab(); }, 50);"><div style="display:flex;align-items:center;gap:6px;"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg><span style="font-size:0.72rem;font-weight:700;color:var(--accent);letter-spacing:0.2px;">SATOSHI'S FAVOR</span></div><div style="display:flex;align-items:center;gap:6px;"><span class="favor-banner-hint" style="font-size:0.65rem;color:var(--text-muted);">Mine now!</span><span style="font-size:0.68rem;color:var(--accent);background:rgba(247,147,26,0.12);border:1px solid rgba(247,147,26,0.2);padding:2px 7px;border-radius:4px;font-family:monospace;font-weight:700;" id="${context}Countdown">--:--:--</span></div></div><style>@keyframes favorPulse{0%,100%{background:rgba(247,147,26,0.08)}50%{background:rgba(247,147,26,0.13)}} @media(max-width:900px){.favor-banner-hint{display:none!important}}</style>`;
        } else {
