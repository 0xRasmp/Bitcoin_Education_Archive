...
(function () {
  ...
  function isForumAdmin() {
    ...
    return (
      email === "needcreations@gmail.com" || email === "info.603btc@gmail.com"
    );
  }
  ...
  var FORUM_RULES = [
    ...
    {
      emoji: '<i class="fa-solid fa-ban"></i>',
    ...
    {
      emoji: '<i class="fa-solid fa-lock"></i>',
      title: "Protect Privacy",
      desc: "Never share personal information about yourself or others — no real names, addresses, phone numbers, or private keys. Stay pseudonymous.",
    },
    {
      emoji: '<i class="fa-solid fa-triangle-exclamation"></i>',
      title: "No Financial Advice",
      desc: 'Don\'t tell others to buy, sell, or invest. Share knowledge and resources, not financial directives. Always add "not financial advice" disclaimers.',
    },
    {
      emoji: '<i class="fa-solid fa-bitcoin-sign"></i>',
      title: "Bitcoin Only",
      desc: 'This is a Bitcoin education forum. Altcoin shilling, ICO promotion, and "next Bitcoin" posts will be removed.',
    },
    {
      emoji: '<i class="fa-solid fa-star"></i>',
      title: "Quality Content",
      desc: "Put effort into your posts. No low-effort single-word posts, all-caps titles, or clickbait. Ask clear questions and provide context.",
    },
    {
      emoji: '<i class="fa-solid fa-robot"></i>',
      title: "No Bot Activity",
      desc: "Automated posting, bot accounts, and AI-generated spam are not allowed. Be a real human participating in real conversations.",
    },
    {
      emoji: '<i class="fa-solid fa-scale-balanced"></i>',
      title: "No Illegal Content",
      desc: "Nothing illegal, including but not limited to: piracy, hacking services, drug markets, weapons, or any content that violates applicable laws.",
    },
    {
      emoji: '<i class="fa-solid fa-shield-halved"></i>',
      title: "Moderators Have Final Say",
      desc: "Posts that violate these rules will be removed without notice. Repeat offenders may be banned. If you see a rule violation, report it.",
    },
  ];

  function showForumRules(force) {
    var html =
      '<div style="position:fixed;inset:0;background:rgba(0,0,0,0.7);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px;" onclick="if(event.target===this){acceptForumRules()}">' +
    ...
    for (var i = 0; i < FORUM_RULES.length; i++) {
      ...
      html +=
        ...
        '<span style="width:28px;flex-shrink:0;margin-top:2px;color:var(--accent);font-size:0.95rem;display:flex;align-items:center;justify-content:center;">' +
        r.emoji +
        "</span>" +
        "<div>" +
        '<div style="color:var(--heading);font-size:0.85rem;font-weight:700;margin-bottom:2px;">' +
        r.title +
        "</div>" +
  ...
  function isCleanText(text) {
    ...
    for (var i = 0; i < words.length; i++) {
      for (var j = 0; j < FORUM_BLOCKED.length; j++) {
        ...
        if (
          FORUM_BLOCKED[j].length >= 4 &&
          words[i].indexOf(FORUM_BLOCKED[j]) !== -1
        )
          return false;
      }
    }
  ...
  window.renderForum = function () {
    ...
    html +=
      ...
      '<button onclick="forumSwitchTab(\'discussions\')" style="flex:1;padding:10px;background:' +
      (forumTab === "discussions" ? "var(--accent)" : "none") +
      ";color:" +
      (forumTab === "discussions" ? "#fff" : "var(--text-muted)") +
      ';border:none;font-size:0.85rem;font-weight:700;cursor:pointer;font-family:inherit;display:inline-flex;align-items:center;justify-content:center;gap:6px;"><i class="fa-solid fa-comments"></i> Discussions</button>' +
      '<button onclick="forumSwitchTab(\'articles\')" style="flex:1;padding:10px;background:' +
      (forumTab === "articles" ? "var(--accent)" : "none") +
      ";color:" +
      (forumTab === "articles" ? "#fff" : "var(--text-muted)") +
      ';border:none;font-size:0.85rem;font-weight:700;cursor:pointer;font-family:inherit;display:inline-flex;align-items:center;justify-content:center;gap:6px;"><i class="fa-solid fa-newspaper"></i> Articles</button>' +
      '<button onclick="forumSwitchTab(\'stacker\')" style="flex:1;padding:10px;background:' +
      (forumTab === "stacker" ? "var(--accent)" : "none") +
      ";color:" +
      (forumTab === "stacker" ? "#fff" : "var(--text-muted)") +
      ';border:none;font-size:0.85rem;font-weight:700;cursor:pointer;font-family:inherit;display:inline-flex;align-items:center;justify-content:center;gap:6px;"><i class="fa-solid fa-bolt"></i> Stacker News</button>' +
      "</div>";
    ...
    if (forumTab === "stacker") {
      ...
      html +=
        '<input id="snSearchInput" type="text" placeholder="Search Stacker News..." value="' +
        snEscape(snSearchQuery) +
        '" onkeydown="if(event.key===\'Enter\')snDoSearch()" style="flex:1;padding:10px 14px;border-radius:10px;border:1px solid var(--border);background:var(--card-bg);color:var(--text);font-size:0.85rem;font-family:inherit;outline:none;" />';
      html +=
        '<button onclick="snDoSearch()" style="padding:10px 16px;border-radius:10px;border:none;background:var(--accent);color:#fff;font-size:0.85rem;font-weight:700;cursor:pointer;font-family:inherit;white-space:nowrap;">🔍</button>';
      if (snSearchQuery) {
        html +=
          '<button onclick="snClearSearch()" style="padding:10px 12px;border-radius:10px;border:1px solid var(--border);background:var(--card-bg);color:var(--text-muted);font-size:0.85rem;cursor:pointer;font-family:inherit;">✕</button>';
      }
      html += "</div>";
      ...
      for (var ti = 0; ti < snTerritories.length; ti++) {
        ...
        html +=
          "<button onclick=\"window._snSub='" +
          t.id +
          "';window._snSearchQuery='';renderForum()\" style=\"padding:5px 12px;border-radius:16px;border:1px solid " +
          (isActive ? "var(--accent)" : "var(--border)") +
          ";background:" +
          (isActive ? "rgba(247,147,26,0.15)" : "var(--card-bg)") +
          ";color:" +
          (isActive ? "var(--accent)" : "var(--text-muted)") +
          ';font-size:0.7rem;cursor:pointer;font-family:inherit;font-weight:600;white-space:nowrap;">' +
          snEscape(t.label) +
          "</button>";
      }
      html += "</div>";
      ...
      if (!snSearchQuery) {
        ...
        for (var ss = 0; ss < snSorts.length; ss++) {
          var s = snSorts[ss];
          html +=
            "<button onclick=\"window._snSort='" +
            s.id +
            '\';renderForum()" style="padding:6px 14px;border-radius:20px;border:1px solid ' +
            (snSort === s.id ? "var(--accent)" : "var(--border)") +
            ";background:" +
            (snSort === s.id ? "var(--accent)" : "var(--card-bg)") +
            ";color:" +
            (snSort === s.id ? "#fff" : "var(--text-muted)") +
        ...
        if (snSort === "top") {
          html +=
            '<select onchange="window._snWhen=this.value;renderForum()" style="padding:6px 10px;border-radius:10px;border:1px solid var(--border);background:var(--card-bg);color:var(--text);font-size:0.75rem;font-family:inherit;">';
          var whens = [
            { id: "day", label: "Today" },
        ...
        html +=
          '<a href="https://stacker.news" target="_blank" rel="noopener noreferrer" style="margin-left:auto;color:var(--text-muted);font-size:0.7rem;text-decoration:none;">stacker.news ↗</a>';
        html += "</div>";
      } else {
        html +=
          '<div style="margin-bottom:14px;font-size:0.8rem;color:var(--text-muted);">Search results for: <strong style="color:var(--text);">' +
          snEscape(snSearchQuery) +
          "</strong></div>";
      }
      ...
      // Feed container
      html +=
        '<div id="snFeed" style="min-height:200px;"><div style="text-align:center;padding:40px 0;color:var(--text-muted);">Loading Stacker News...</div></div>';
      html += "</div>";
      fc.innerHTML = html;
      if (snSearchQuery) {
    ...
    ["recent", "top", "discussed"].forEach(function (s) {
      var labels = {
        recent: '<i class="fa-solid fa-clock"></i> Recent',
        top: '<i class="fa-solid fa-fire"></i> Top',
        discussed: '<i class="fa-solid fa-comments"></i> Discussed',
      };
  ...
  async function forumLoadPostsFallback() {
    ...
    try {
      ...
      if (
        window._forumCache &&
        window._forumCache.key === _forumKey &&
        _forumNow - window._forumCache.time < 120000
      ) {
    ...
    posts.forEach(function (p) {
      var lv =
        typeof getLevel === "function"
          ? getLevel(p.authorPoints || 0)
          : { emoji: "🟢" };
      var cat = FORUM_CATEGORIES.find(function (c) {
        return c.id === p.category;
      });
      var catLabel = cat ? cat.emoji + " " + cat.name : "";
      var isOwn =
        auth && auth.currentUser && p.authorId === auth.currentUser.uid;
      var canDelete = isOwn || isForumAdmin();
      var hasVoted =
        p.voters &&
        auth &&
        auth.currentUser &&
        p.voters.indexOf(auth.currentUser.uid) !== -1;
      ...
      html +=
        ...
        "<button onclick=\"event.stopPropagation();forumVotePost('" +
        p.id +
        '\')" style="background:none;border:none;cursor:pointer;font-size:1rem;padding:4px;color:' +
        (hasVoted ? "var(--accent)" : "var(--text-faint)") +
        ';touch-action:manipulation;" title="Upvote"><i class="fa-solid fa-thumbs-up"></i></button>' +
        '<span style="color:' +
        (hasVoted ? "var(--accent)" : "var(--text-muted)") +
        ';font-size:0.8rem;font-weight:700;">' +
        (p.upvotes || 0) +
        "</span>" +
        "</div>" +
        '<div style="flex:1;min-width:0;overflow:hidden;">' +
        '<div style="color:var(--heading);font-size:0.9rem;font-weight:700;margin-bottom:3px;line-height:1.3;word-wrap:break-word;">' +
        fEsc(p.title) +
        "</div>" +
        (p.body
          ? '<div style="color:var(--text-muted);font-size:0.75rem;line-height:1.4;margin-bottom:5px;overflow:hidden;max-height:2.6em;word-wrap:break-word;">' +
            fEsc(p.body).substring(0, 120) +
            (p.body.length > 120 ? "..." : "") +
            "</div>"
          : "") +
        '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">' +
        "<span onclick=\"event.stopPropagation();if(typeof showUserProfile==='function')showUserProfile('" +
        p.authorId +
        '\')" style="font-size:0.7rem;' +
        (p.authorFaction
          ? window._factionNameStyle(p.authorFaction)
          : "color:var(--text-faint)") +
        ';cursor:pointer;transition:0.2s;">' +
        lv.emoji +
        " " +
        fEsc(p.authorName || "Anon") +
        "</span>" +
        '<span style="font-size:0.65rem;color:var(--text-faint);">' +
        timeAgo(p.createdAt) +
        "</span>" +
        '<span style="font-size:0.7rem;color:var(--text-faint);display:inline-flex;align-items:center;gap:3px;"><i class="fa-solid fa-comment"></i> ' +
        (p.replyCount || 0) +
        "</span>" +
        (catLabel
        ...
        (canDelete
          ? "<button onclick=\"event.stopPropagation();forumDeletePost('" +
            p.id +
            '\')" style="background:none;border:none;color:var(--text-faint);font-size:0.7rem;cursor:pointer;padding:4px;touch-action:manipulation;opacity:0.5;" title="Delete post"><i class="fa-solid fa-trash"></i></button>'
          : "") +
        "</div>" +
        "</div>" +
        "</div>" +
  ...
  // ---- Sort/Filter ----
  window.forumSetSort = function (s) {
    forumSort = s;
    renderForum();
  };
  window.forumSetCategory = function (c) {
    forumCategory = c;
    sessionStorage.setItem("btc_forum_category", c);
    renderForum();
  };
  ...
  window.forumViewPost = async function (postId, fromPopState) {
    ...
    if (!fromPopState)
      history.pushState(
        { channel: "forum", forumPost: postId },
        "",
        "#forum/post/" + postId,
      );
    fc.innerHTML =
      '<div style="max-width:700px;margin:0 auto;padding:20px 16px;"><div style="text-align:center;padding:100px 40px;display:flex;flex-direction:column;gap:12px;">' +
    ...
    try {
      ...
      html +=
        ...
        '<h2 style="color:var(--heading);font-size:1.15rem;font-weight:800;margin:0 0 8px;line-height:1.3;word-wrap:break-word;">' +
        fEsc(p.title) +
        "</h2>" +
        ...
        ";cursor:pointer;\" onclick=\"if(typeof showUserProfile==='function')showUserProfile('" +
        p.authorId +
        "')\">" +
        lv.emoji +
        " " +
      ...
      html +=
        ...
        "<button onclick=\"forumVotePost('" +
        p.id +
        '\')" style="display:flex;align-items:center;gap:4px;background:' +
        (hasVoted ? "var(--accent-bg)" : "none") +
        ";border:1px solid " +
        (hasVoted ? "var(--accent)" : "var(--border)") +
      ...
      if (auth && auth.currentUser && !auth.currentUser.isAnonymous) {
        html +=
          ...
          "<button onclick=\"forumSubmitReply('" +
          p.id +
          '\')" style="padding:10px 20px;background:var(--accent);color:#fff;border:none;border-radius:10px;font-size:0.9rem;font-weight:700;cursor:pointer;font-family:inherit;touch-action:manipulation;flex-shrink:0;">Reply</button>' +
          "</div></div>";
      } else {
        html +=
          '<div style="text-align:center;padding:20px;color:var(--text-muted);font-size:0.9rem;margin-bottom:20px;background:var(--card-bg);border:2px solid var(--accent);border-radius:12px;">' +
  ...
  async function forumLoadReplies(postId) {
    ...
    try {
      ...
      function _renderReplyCard(r, isNested) {
        var rlv =
          typeof getLevel === "function"
            ? getLevel(r.authorPoints || 0)
            : { emoji: "\uD83D\uDFE2" };
        var hasVotedR =
          r.voters &&
          auth &&
          auth.currentUser &&
          r.voters.indexOf(auth.currentUser.uid) !== -1;
        ...
        bodyHtml = forumRenderMentions(bodyHtml);
        var canDeleteReply =
          (auth && auth.currentUser && r.authorId === auth.currentUser.uid) ||
          isForumAdmin();
        var canReply =
          auth && auth.currentUser && !auth.currentUser.isAnonymous;
        var replyToLabel = r.parentAuthorName
          ? '<div style="font-size:0.7rem;color:var(--accent);margin-bottom:4px;">\u21A9 ' +
            fEsc(r.parentAuthorName) +
            "</div>"
          : "";
        var indent = isNested
          ? "margin-left:24px;border-left:2px solid var(--accent);"
          : "";
        var out =
          '<div style="background:var(--card-bg);border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:8px;word-wrap:break-word;overflow-wrap:break-word;' +
          indent +
          '">' +
          replyToLabel +
          ...
          '<span style="font-size:0.8rem;' +
          (r.authorFaction
            ? window._factionNameStyle(r.authorFaction)
            : "color:var(--text-muted)") +
          ";cursor:pointer;\" onclick=\"if(typeof showUserProfile==='function')showUserProfile('" +
          r.authorId +
          "')\">" +
          rlv.emoji +
          " " +
          fEsc(r.authorName || "Anon") +
          "</span>" +
          '<span style="font-size:0.7rem;color:var(--text-faint);">' +
          timeAgo(r.createdAt) +
          "</span>" +
          "</div>" +
          '<div style="color:var(--text);font-size:0.85rem;line-height:1.5;margin-bottom:8px;">' +
          bodyHtml +
          "</div>" +
          '<div style="display:flex;gap:8px;align-items:center;">' +
          "<button onclick=\"forumVoteReply('" +
          r.id +
          '\')" style="display:flex;align-items:center;gap:4px;background:' +
          (hasVotedR ? "var(--accent-bg)" : "none") +
          ";border:1px solid " +
          (hasVotedR ? "var(--accent)" : "var(--border)") +
          ";border-radius:12px;padding:3px 10px;cursor:pointer;color:" +
          (hasVotedR ? "var(--accent)" : "var(--text-faint)") +
          ...
          (canReply
            ? '<button data-reply-id="' +
              ...
              '" data-reply-author="' +
              fEsc(r.authorName || "Anon") +
              '" onclick="window.forumReplyToReply(this.dataset.replyId,this.dataset.replyAuthor)" style="display:flex;align-items:center;gap:4px;background:none;border:1px solid var(--border);border-radius:12px;padding:3px 10px;cursor:pointer;color:var(--text-faint);font-size:0.75rem;font-family:inherit;touch-action:manipulation;">\u21A9 Reply</button>'
            : "") +
          (canDeleteReply
      ...
      Object.keys(childMap).forEach(function (parentId) {
        var parentExists = topLevel.some(function (t) {
  ...
  window.forumNewPost = function () {
    ...
    // Category
    html +=
      '<label style="color:var(--text-muted);font-size:0.8rem;display:block;margin-bottom:4px;">Category</label>' +
      '<select id="forumNewCat" style="' +
      _inputStyle +
      'cursor:pointer;">';
    FORUM_CATEGORIES.forEach(function (c) {
      html +=
    ...
    // Title
    html +=
      '<label style="color:var(--text-muted);font-size:0.8rem;display:block;margin-bottom:4px;">Title *</label>' +
      '<input type="text" id="forumNewTitle" maxlength="120" placeholder="What\'s on your mind?" style="' +
      _inputStyle +
      '">';

    // Body
    html +=
      '<label style="color:var(--text-muted);font-size:0.8rem;display:block;margin-bottom:4px;">Details <span style="color:var(--text-faint);">(optional, 2000 chars max)</span></label>' +
      '<textarea id="forumNewBody" rows="5" maxlength="2000" placeholder="Share your thoughts, ask a question, start a discussion..." style="' +
      _inputStyle +
      'resize:vertical;min-height:100px;"></textarea>';

    // Link
    html +=
      '<label style="color:var(--text-muted);font-size:0.8rem;display:block;margin-bottom:4px;">Link <span style="color:var(--text-faint);">(optional)</span></label>' +
      '<input type="url" id="forumNewLink" maxlength="200" placeholder="https://..." style="' +
      _inputStyle +
      '">';
  ...
  window.forumSubmitPost = async function () {
    ...
    if (_now - _lastForumPostTime < 30000) {
      if (typeof showToast === "function")
        showToast("⏳ Wait 30 seconds between posts");
      return;
    }
    ...
    if (
      ...
      Date.now() - window._forumFormOpenTime < 5000
    ) {
      if (status)
        status.innerHTML =
    ...
    try {
      ...
      if (typeof currentUser !== "undefined" && currentUser)
        currentUser.forumPosts = (currentUser.forumPosts || 0) + 1;
      // Raid Boss: forum post
      if (typeof window._raidOnForumPost === "function")
        window._raidOnForumPost();
  ...
  window.forumSubmitReply = async function (postId) {
    ...
    var input = document.getElementById("forumReplyInput");
    if (!auth || !auth.currentUser || auth.currentUser.isAnonymous || !input) {
    ...
    try {
      ...
      if (replyingTo && replyingTo.replyId) {
        ...
        try {
          var parentReplyDoc = await db
            .collection("forum_replies")
            .doc(replyingTo.replyId)
            .get();
          if (
            parentReplyDoc.exists &&
            parentReplyDoc.data().authorId &&
            ...
            sendNotification(
              ...
              _un +
                ...
                (forumCurrentPost
                  ? (forumCurrentPost.title || "").substring(0, 40)
                  : "a post") +
                '"',
              "forum_post",
              postId,
      ...
      forumNotifyMentions(
        ...
        "forum_post",
        postId,
        forumCurrentPost ? forumCurrentPost.title : "",
      );
      // Track for badge
      db.collection("users")
        .doc(auth.currentUser.uid)
      ...
      if (typeof currentUser !== "undefined" && currentUser)
        currentUser.forumReplies = (currentUser.forumReplies || 0) + 1;
      input.value = "";
  ...
  window.forumDeletePost = async function (postId) {
    ...
    try {
      ...
      var repliesSnap = await db
        .collection("forum_replies")
        .where("postId", "==", postId)
        .get();
      var batch = db.batch();
      repliesSnap.forEach(function (doc) {
  ...
  function mdToHtml(md) {
    ...
    h = h.replace(
      /!\[([^\]]*)\]\(([^)]+)\)/g,
      '<img src="$2" alt="$1" style="max-width:100%;border-radius:12px;margin:16px 0;display:block;">',
    );
    // Links: [text](url)
    h = h.replace(
      /\[([^\]]+)\]\(([^)]+)\)/g,
      '<a href="$2" target="_blank" rel="noopener" style="color:var(--accent);text-decoration:underline;font-weight:600;">$1</a>',
    );
  ...
  async function loadArticles() {
    ...
    try {
      ...
      articles.forEach(function (a) {
        ...
        // Meta
        html +=
          '<div style="display:flex;align-items:center;gap:8px;font-size:0.75rem;color:var(--text-faint);">' +
          "<span " +
          (a.authorFaction
            ? 'style="' + window._factionNameStyle(a.authorFaction) + '"'
            : "") +
          ">" +
          lv.emoji +
          " " +
          fEsc(a.authorName || "Anon") +
          "</span>" +
          "<span>·</span><span>" +
          readTime +
          " min read</span>" +
        ...
        // Stats
        html +=
          '<div style="display:flex;gap:12px;margin-top:8px;font-size:0.75rem;color:var(--text-faint);">' +
          "<span>👍 " +
          (a.upvotes || 0) +
          "</span>" +
          "<span>💬 " +
  ...
  window.articleNew = function () {
    ...
    var draft = null;
    try {
      draft = JSON.parse(localStorage.getItem("btc_article_draft"));
    } catch (e) {}
    ...
    html +=
      '<label style="color:var(--text-muted);font-size:0.8rem;display:block;margin-bottom:4px;">Title *</label>' +
      '<input type="text" id="articleTitle" maxlength="120" placeholder="Your article title" value="' +
      (draft && draft.title ? fEsc(draft.title) : "") +
      '" style="' +
      _s +
      '">';
    ...
    // Subtitle
    html +=
      '<label style="color:var(--text-muted);font-size:0.8rem;display:block;margin-bottom:4px;">Subtitle <span style="color:var(--text-faint);">(optional)</span></label>' +
      '<input type="text" id="articleSubtitle" maxlength="200" placeholder="Brief description" value="' +
      (draft && draft.subtitle ? fEsc(draft.subtitle) : "") +
      '" style="' +
      _s +
      '">';
    ...
    // Tags
    html +=
      '<label style="color:var(--text-muted);font-size:0.8rem;display:block;margin-bottom:6px;">Tags (pick 1-3) *</label>' +
    ...
    // Cover image
    html +=
      '<label style="color:var(--text-muted);font-size:0.8rem;display:block;margin-bottom:4px;">Cover Image <span style="color:var(--text-faint);">(optional)</span></label>' +
      '<input type="file" id="articleCover" accept="image/jpeg,image/png,image/webp" style="' +
      _s +
      'font-size:0.8rem;">';

    // Terms
    html +=
      '<div style="background:rgba(247,147,26,0.05);border:1px dashed var(--accent);border-radius:10px;padding:12px;margin:12px 0;">' +
  ...
  window.toggleArticleTag = function (btn, tag) {
    var sel =
      btn.style.borderColor.indexOf("var(--accent)") !== -1 ||
      btn.style.color.indexOf("var(--accent)") !== -1;
    if (sel) {
      ...
      if (count >= 3) {
        if (typeof showToast === "function") showToast("Max 3 tags");
        return;
      }
  ...
  window.articleInsertLink = function () {
    ...
    dialog.style.cssText =
      "position:fixed;inset:0;z-index:99999;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.6);padding:16px;";
    dialog.onclick = function (e) {
      if (e.target === dialog) dialog.remove();
    };
    var is =
      "width:100%;padding:10px 12px;background:var(--card-bg);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:0.9rem;font-family:inherit;box-sizing:border-box;margin-bottom:10px;";
    dialog.innerHTML =
      ...
      '<input type="text" id="linkDialogText" value="' +
      (typeof fEsc === "function" ? fEsc(selected) : selected) +
      '" placeholder="Click here to read more" style="' +
      is +
      '">' +
      '<label style="display:block;font-size:0.75rem;color:var(--text-muted);margin-bottom:4px;">URL</label>' +
      '<input type="url" id="linkDialogUrl" placeholder="https://example.com" style="' +
      is +
      '">' +
      '<div style="display:flex;gap:8px;">' +
    ...
    setTimeout(function () {
      document.getElementById("linkDialogUrl").focus();
    }, 100);
  };

  window.articleDoInsertLink = function () {
    var text =
      (document.getElementById("linkDialogText").value || "").trim() || "link";
    var url = (document.getElementById("linkDialogUrl").value || "").trim();
    if (!url) {
      if (typeof showToast === "function") showToast("Enter a URL");
      return;
    }
  ...
  window.articleInsertImage = function () {
    ...
    dialog.innerHTML =
      ...
      '<input type="url" id="imageDialogUrl" placeholder="https://example.com/photo.jpg" style="' +
      is +
      '">' +
      ...
      '<input type="text" id="imageDialogAlt" placeholder="Describe the image" style="' +
      is +
      '">' +
  ...
  window.articleDoInsertImage = function () {
    ...
    var alt =
      (document.getElementById("imageDialogAlt").value || "").trim() || "image";
    if (!url) {
      if (typeof showToast === "function")
        showToast("Enter an image URL or upload a file");
      return;
    }
    var ta = document.getElementById("articleBody");
    if (ta) {
  ...
  window.articleSaveDraft = function (silent) {
    ...
    document
      ...
      .forEach(function (b) {
        if (b.style.color && b.style.color.indexOf("var(--accent)") !== -1)
          tags.push(b.getAttribute("data-tag"));
      });
  ...
  window.articleSubmit = async function () {
    ...
    if (!isCleanText(title) || !isCleanText(body)) {
      if (status)
        status.innerHTML =
    ...
    try {
      ...
      if (coverInput && coverInput.files && coverInput.files[0]) {
        ...
        try {
          ...
          var ref = storage.ref(
            "articles/" + auth.currentUser.uid + "/" + Date.now() + "_cover",
          );
      ...
      var _ac = parseInt(localStorage.getItem("btc_articles_published") || "0");
      localStorage.setItem("btc_articles_published", String(_ac + 1));
    ...
    try {
      ...
      var dateStr =
        a.createdAt && a.createdAt.toDate
          ...
          : "";
      var isAuthor =
        auth && auth.currentUser && auth.currentUser.uid === a.authorId;
      var isAdmin = isForumAdmin();
      var hasVoted =
        auth &&
        auth.currentUser &&
        (a.voters || []).indexOf(auth.currentUser.uid) !== -1;
      ...
      if (a.coverUrl) {
        html +=
          '<div style="margin:-16px -12px 20px;border-radius:0 0 16px 16px;overflow:hidden;"><img src="' +
          fEsc(a.coverUrl) +
          '" alt="Cover" style="width:100%;display:block;object-fit:contain;max-height:500px;"></div>';
      }
      ...
      html +=
        ...
        (a.authorFaction
          ? window._factionNameStyle(a.authorFaction)
          : "color:var(--text)") +
        ";cursor:pointer;\" onclick=\"if(typeof showUserProfile==='function')showUserProfile('" +
        a.authorId +
        "')\">" +
      ...
      if (auth && auth.currentUser && !auth.currentUser.isAnonymous) {
        html +=
          ...
          "<button onclick=\"articleSubmitReply('" +
          articleId +
          '\')" style="margin-top:6px;padding:10px 20px;background:var(--accent);color:#fff;border:none;border-radius:10px;font-size:0.85rem;font-weight:700;cursor:pointer;font-family:inherit;">Post Comment</button>' +
      ...
      if (voters.indexOf(uid) !== -1) {
        ...
        if (_ad.authorId && typeof sendNotification === "function") {
          ...
          sendNotification(
            ...
            "upvote",
            _un +
              ' upvoted your article "' +
              (_ad.title || "").substring(0, 40) +
              '"',
            "article",
            articleId,
          );
    ...
    try {
      ...
      snap.forEach(function (doc) {
        ...
        var canDel =
          auth &&
          auth.currentUser &&
          (auth.currentUser.uid === r.authorId || isForumAdmin());
        html +=
  ...
  window.articleSubmitReply = async function (articleId) {
    ...
    if (!input || !auth || !auth.currentUser || auth.currentUser.isAnonymous)
      return;
    var body = input.value.trim();
    if (!body || body.length < 2) {
      if (status)
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          sendNotification(
            ...
            _un +
              ...
              (_aDoc.data().title || "").substring(0, 40) +
              '"',
            "article",
            articleId,
          );
        ...
        forumNotifyMentions(
          ...
          "article",
          articleId,
          _aDoc && _aDoc.exists ? _aDoc.data().title : "",
        );
      } catch (e) {}
      input.value = "";
      if (status)
  ...
  window.articleEdit = async function (articleId) {
    try {
      ...
      setTimeout(function () {
        ...
        if (btn) {
          btn.textContent = "✏️ Update Article";
          btn.setAttribute("onclick", "articleUpdate()");
        }
      }, 100);
    } catch (e) {
      if (typeof showToast === "function")
  ...
  window.articleUpdate = async function () {
    ...
    if (wordCount < 500) {
      if (status)
        status.innerHTML = '<span style="color:#ef4444;">Min 500 words</span>';
      return;
    }
    ...
    try {
      ...
      if (coverInput && coverInput.files && coverInput.files[0]) {
        ...
        if (file.size > 3 * 1024 * 1024) {
          if (status)
            status.innerHTML =
              '<span style="color:#ef4444;">Cover too large (max 3MB)</span>';
          return;
        }
        if (status)
          status.innerHTML =
      ...
      if (status)
        status.innerHTML = '<span style="color:#ef4444;">Error updating</span>';
    }
  };
  ...
  window.articleTogglePin = async function (articleId, pinState) {
    try {
      ...
      if (typeof showToast === "function")
        showToast(pinState ? "📌 Article pinned!" : "📌 Article unpinned");
      articleView(articleId);
    } catch (e) {}
  };
  ...
  window.articleDelete = async function (articleId) {
    ...
    try {
      ...
      if (typeof showToast === "function") showToast("Error deleting article");
    }
  };
  ...
  window.loadStackerNewsFeed = function (sort, when, sub) {
    ...
    if (!feed) return;
    var url =
      "https://embed-proxy.needcreations.workers.dev/api/sn?sort=" +
    ...
    fetch(url)
      ...
      .then(function (data) {
        if (!data || !data.data || !data.data.items || !data.data.items.items) {
          feed.innerHTML =
            '<div style="text-align:center;padding:40px 0;color:var(--text-muted);">Could not load Stacker News feed</div>';
          return;
        }
        var items = data.data.items.items;
        if (!items.length) {
          feed.innerHTML =
            '<div style="text-align:center;padding:40px 0;color:var(--text-muted);">No posts found</div>';
          return;
        }
      ...
      .catch(function () {
        feed.innerHTML =
          '<div style="text-align:center;padding:40px 0;color:var(--text-muted);">Failed to load feed. <button onclick="loadStackerNewsFeed(\'' +
          sort +
          "','" +
          when +
          "','" +
          (sub || "") +
          '\')" style="color:var(--accent);background:none;border:none;cursor:pointer;font-family:inherit;text-decoration:underline;">Retry</button></div>';
      });
  };
  ...
  function snSearchFeed(q) {
    ...
    if (!feed) return;
    var url =
      "https://embed-proxy.needcreations.workers.dev/api/sn/search?q=" +
    ...
    fetch(url)
      ...
      .then(function (data) {
        if (
          ...
          feed.innerHTML =
            '<div style="text-align:center;padding:40px 0;color:var(--text-muted);">No results found</div>';
          return;
        }
        var items = data.data.search.items;
        if (!items.length) {
          feed.innerHTML =
            '<div style="text-align:center;padding:40px 0;color:var(--text-muted);">No results found for \"' +
            snEscape(q) +
            '\"</div>';
          return;
        }
      ...
      .catch(function () {
        feed.innerHTML =
          '<div style="text-align:center;padding:40px 0;color:var(--text-muted);">Search failed. <button onclick="snDoSearch()" style="color:var(--accent);background:none;border:none;cursor:pointer;font-family:inherit;text-decoration:underline;">Retry</button></div>';
      });
  }
  ...
  function snRenderFeedCards(items) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      h +=
        '<div onclick="snOpenPost(' +
        item.id +
        ")\" style=\"cursor:pointer;padding:14px 16px;border:1px solid var(--border);border-radius:12px;margin-bottom:8px;background:var(--card-bg);transition:border-color 0.2s,transform 0.1s;position:relative;\" onmouseover=\"this.style.borderColor='var(--accent)';this.style.transform='translateY(-1px)'\" onmouseout=\"this.style.borderColor='var(--border)';this.style.transform='none'\">";
      ...
      h +=
        ...
        snFormatSats(item.sats) +
        "</div>";
      h +=
        '<div style="font-size:0.6rem;color:var(--text-muted);margin-top:1px;">sats</div>';
      h += "</div>";
      // Content
      h += '<div style="flex:1;min-width:0;">';
      h +=
        '<div style="font-size:0.9rem;font-weight:600;color:var(--heading);line-height:1.35;word-break:break-word;">' +
        snEscape(item.title) +
        "</div>";
      h +=
        '<div style="display:flex;flex-wrap:wrap;gap:6px;align-items:center;margin-top:6px;font-size:0.7rem;color:var(--text-muted);">';
      // Clickable username
      h +=
        '<a href="https://stacker.news/' +
        snEscape(item.user.name) +
        '" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()" style="color:var(--text-muted);text-decoration:none;" onmouseover="this.style.color=\'var(--accent)\'" onmouseout="this.style.color=\'var(--text-muted)\'">@' +
        snEscape(item.user.name) +
        "</a>";
      h += "<span>\u00b7</span>";
      h += "<span>" + ago + "</span>";
      ...
      if (subName) {
        h +=
          '<span style="padding:2px 8px;border-radius:10px;background:rgba(99,102,241,0.12);color:#818cf8;font-size:0.65rem;font-weight:600;">' +
          snEscape(subName) +
          "</span>";
      }
      h += "</div></div>";
      // View on SN link
      h +=
        '<a href="https://stacker.news/items/' +
        item.id +
        '" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()" style="position:absolute;top:12px;right:12px;color:var(--text-muted);font-size:0.8rem;text-decoration:none;opacity:0.5;padding:4px;" onmouseover="this.style.opacity=\'1\';this.style.color=\'var(--accent)\'" onmouseout="this.style.opacity=\'0.5\';this.style.color=\'var(--text-muted)\'">\u2197</a>';
      h += "</div></div>";
    }
    return h;
  }
  ...
  window.snOpenPost = function (id) {
    ...
    fetch("https://embed-proxy.needcreations.workers.dev/api/sn/item?id=" + id)
      ...
      .catch(function () {
        fc.innerHTML =
          '<div style="max-width:700px;margin:0 auto;padding:16px 12px;"><div style="text-align:center;padding:40px 0;color:var(--text-muted);">Failed to load post. <button onclick="snOpenPost(' +
          id +
          ')" style="color:var(--accent);background:none;border:none;cursor:pointer;font-family:inherit;text-decoration:underline;">Retry</button> · <button onclick="forumTab=\'stacker\';renderForum()" style="color:var(--accent);background:none;border:none;cursor:pointer;font-family:inherit;text-decoration:underline;">Back</button></div></div>';
      });
  };
  ...
  function snRenderPost(fc, item) {
    ...
    // Back button
    h +=
      '<button onclick="forumTab=\'stacker\';renderForum()" style="background:none;border:none;color:var(--text-muted);font-size:0.85rem;cursor:pointer;padding:8px 0;margin-bottom:12px;font-family:inherit;touch-action:manipulation;">← Back to Stacker News</button>';

    // Post card
    h +=
      '<div style="border:1px solid var(--border);border-radius:16px;background:var(--card-bg);padding:20px;margin-bottom:16px;">';

    // Title
    h +=
      '<h2 style="font-size:1.2rem;font-weight:800;color:var(--heading);margin:0 0 10px;line-height:1.3;">' +
      snEscape(item.title) +
      "</h2>";

    // Meta row
    h +=
      '<div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin-bottom:14px;font-size:0.75rem;color:var(--text-muted);">';
    h +=
    ...
    h +=
      '<a href="https://stacker.news/' +
      snEscape(item.user.name) +
      '" target="_blank" rel="noopener noreferrer" style="color:var(--text-muted);text-decoration:none;" onmouseover="this.style.color=\'var(--accent)\'" onmouseout="this.style.color=\'var(--text-muted)\'">@' +
      snEscape(item.user.name) +
      "</a>";
    h += "<span>·</span>";
    ...
    if (item.url) {
      ...
      h +=
        '<a href="' +
        snEscape(item.url) +
        '" target="_blank" rel="noopener noreferrer" style="display:inline-block;padding:8px 16px;background:rgba(247,147,26,0.1);border:1px solid rgba(247,147,26,0.3);border-radius:8px;color:var(--accent);font-size:0.8rem;text-decoration:none;margin-bottom:14px;word-break:break-all;">🔗 ' +
        snEscape(domain || item.url) +
        " ↗</a>";
    }
    ...
    if (item.text) {
      h +=
        '<div style="font-size:0.9rem;line-height:1.65;color:var(--text);word-break:break-word;">' +
        snRenderMarkdown(item.text) +
        "</div>";
    }
    ...
    h +=
      '<a href="https://stacker.news/items/' +
      item.id +
      '" target="_blank" rel="noopener noreferrer" style="color:var(--text-muted);font-size:0.75rem;text-decoration:none;">View on stacker.news ↗</a>';
    h += "</div>";
    ...
    if (
      ...
      h +=
        '<div style="font-size:0.85rem;font-weight:700;color:var(--heading);margin-bottom:12px;">💬 Comments (' +
        item.ncomments +
        ")</div>";
  ...
  function snRenderComments(comments, depth) {
    ...
    for (var i = 0; i < comments.length; i++) {
      ...
      h +=
        ...
        indent +
        'px;padding:12px 14px;border:1px solid var(--border);border-radius:10px;margin-bottom:6px;background:var(--card-bg);">';
      h +=
        '<div style="display:flex;gap:8px;align-items:center;margin-bottom:6px;font-size:0.7rem;color:var(--text-muted);">';
      h +=
        '<a href="https://stacker.news/' +
        snEscape(c.user.name) +
        '" target="_blank" rel="noopener noreferrer" style="font-weight:600;color:var(--text-muted);text-decoration:none;" onmouseover="this.style.color=\'var(--accent)\'" onmouseout="this.style.color=\'var(--text-muted)\'">@' +
        snEscape(c.user.name) +
        "</a>";
      h += "<span>·</span>";
      ...
      if (c.text) {
        h +=
          '<div style="font-size:0.82rem;line-height:1.55;color:var(--text);word-break:break-word;">' +
          snRenderMarkdown(c.text) +
          "</div>";
      }
      h += "</div>";
      // Nested replies
  ...
  function snRenderMarkdown(text) {
    ...
    s = s.replace(
      /^#{3}\s+(.+)$/gm,
      '<div style="font-size:1rem;font-weight:700;margin:14px 0 6px;color:var(--heading);">$1</div>',
    );
    s = s.replace(
      /^#{2}\s+(.+)$/gm,
      '<div style="font-size:1.05rem;font-weight:700;margin:16px 0 8px;color:var(--heading);">$1</div>',
    );
    s = s.replace(
      /^#{1}\s+(.+)$/gm,
      '<div style="font-size:1.1rem;font-weight:800;margin:18px 0 8px;color:var(--heading);">$1</div>',
    );
    // Bold + italic
    ...
    s = s.replace(
      /`([^`]+)`/g,
      '<code style="background:rgba(255,255,255,0.06);padding:1px 5px;border-radius:4px;font-size:0.85em;">$1</code>',
    );
    // Line breaks
  ...
  function forumRenderMentions(html) {
    ...
    return html.replace(
      /(<[^>]+>)|(@([a-zA-Z0-9_\-\.]{2,20}))/g,
      function (match, tag, mention, username) {
        if (tag) return tag; // HTML tag — pass through unchanged
        // It's an @mention in a text node
        return (
          '<span class="forum-mention" onclick="forumMentionClick(\'' +
          username.replace(/'/g, "") +
          "')\">" +
          mention +
          "</span>"
      ...
      .then(function (snap) {
        if (!snap.empty) {
          if (typeof showUserProfile === "function")
            showUserProfile(snap.docs[0].id);
        } else {
          ...
          db.collection("users")
            ...
            .then(function (snap2) {
              if (!snap2.empty) {
                if (typeof showUserProfile === "function")
                  showUserProfile(snap2.docs[0].id);
              } else {
                if (typeof showToast === "function")
  ...
  window.forumNotifyMentions = async function (
    body,
    contextType,
    contextId,
    contextTitle,
  ) {
    if (!body || typeof db === "undefined" || !auth || !auth.currentUser)
      return;
    ...
    for (var i = 0; i < unique.length; i++) {
      try {
        var snap = await db
          .collection("users")
          .where("username", "==", unique[i])
          .limit(1)
          .get();
        if (snap.empty) {
          snap = await db
            .collection("users")
            .where("username_lower", "==", unique[i].toLowerCase())
            .limit(1)
            .get();
        }
        if (!snap.empty) {
          ...
          if (uid !== myUid && typeof sendNotification === "function") {
            sendNotification(
              uid,
              "mention",
              myName + ' mentioned you in "' + titleSnip + '"',
              contextType,
              contextId,
            );
          }
        ...
        console.warn("[forum] Mention notify error:", e);
      }
    }
  };
  ...
  function mentionShowDropdown(textarea, users) {
    ...
    dd.style.cssText =
      "position:absolute;z-index:9999;background:var(--card-bg,#1a1a2e);border:1px solid var(--border);border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,0.4);overflow:hidden;max-height:220px;overflow-y:auto;min-width:200px;";
    ...
    users.forEach(function (u, idx) {
      ...
      item.innerHTML =
        ...
        "</span>" +
        (u.points
          ? '<span style="color:var(--text-faint);font-size:0.7rem;margin-left:6px;">' +
            (u.points || 0).toLocaleString() +
            " XP</span>"
          : "");
      item.style.cssText =
        "padding:10px 14px;cursor:pointer;display:flex;align-items:center;gap:8px;font-size:0.85rem;color:var(--text);transition:background 0.15s;" +
  ...
  function mentionHighlight() {
    ...
    items.forEach(function (el, i) {
      el.style.background =
        i === _mentionState.selectedIdx ? "var(--accent-bg)" : "";
    });
  }
      ...
      db.collection("users")
        ...
        .then(function () {
          ...
          if (done >= variants.length) {
            ...
            if (_mentionState.activeTextarea)
              mentionShowDropdown(
                _mentionState.activeTextarea,
                allUsers.slice(0, 5),
              );
          }
        });
  ...
  function mentionBindTextarea(textarea) {
    ...
    textarea.addEventListener("keydown", function (e) {
      ...
      if (e.key === "ArrowDown") {
        e.preventDefault();
        _mentionState.selectedIdx = Math.min(
          _mentionState.selectedIdx + 1,
          _mentionState.results.length - 1,
        );
        mentionHighlight();
        ...
        if (
          _mentionState.results.length > 0 &&
          _mentionState.selectedIdx >= 0
        ) {
          e.preventDefault();
  ...
  _mentionStyle.textContent =
    ".forum-mention{color:var(--accent);cursor:pointer;font-weight:600;transition:opacity 0.15s;}.forum-mention:hover{opacity:0.8;text-decoration:underline;}";
