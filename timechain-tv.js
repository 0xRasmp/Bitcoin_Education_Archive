...
var STATIONS = [
        {
            "id": "pleb-underground",
            "name": "Pleb Underground Unlimited",
            "emoji": "🏴",
            "desc": "Bitcoin only! Daily live shows, discussions & everything Bitcoin from Pleb Underground",
            "color": "#ff6b00",
            "videos": [
                ...
                {"id":"KHJwLXUIloI","title":"Pleb Underground Monthly Live Episode #8","duration":9000},
                ...
                {"id":"0-dJPimO_MU","title":"Here's How PayJoin Is Solving Bitcoins Privacy Problem! | Guest: Dan Gould | EP 63","duration":4000},
...
function joinStation(stationId) {
    ...
    if (prevJoined !== stationId) {
        ...
        try {
            ...
            var _sv = JSON.parse(localStorage.getItem('btc_tctv_stations_visited_' + _t) || '[]');
            if (_sv.indexOf(stationId) === -1) _sv.push(stationId);
            localStorage.setItem('btc_tctv_stations_visited_' + _t, JSON.stringify(_sv));
        } catch(e) {}
    }
...
// ── Pleb Underground Live Override ──
var _plebLiveOverride = null; // { videoId, title } when PU is live
var _plebLivePollInterval = null;
var _plebLiveActive = false; // true when we're currently showing a live stream

// Worker URL — update after deploying the pleb-underground-sync worker
var _PLEB_WORKER_URL = 'https://pleb-underground-sync.needcreations.workers.dev';

function _plebLiveCheck() {
    // Poll the Pleb Underground worker for live status
    fetch(_PLEB_WORKER_URL + '/live')
        .then(function(r) { return r.ok ? r.json() : null; })
        .then(function(data) {
            if (!data) { _plebLiveClear(); return; }

            if (data.isLive && data.videoId) {
                // Live stream detected!
                if (!_plebLiveActive || (_plebLiveOverride && _plebLiveOverride.videoId !== data.videoId)) {
                    _plebLiveOverride = { videoId: data.videoId, title: data.title || 'Pleb Underground LIVE' };
                    _plebLiveActive = true;
                    // Only override if currently on Channel 0
                    if (_currentStation === 'pleb-underground') {
                        _plebLivePlay(data.videoId, data.title);
                    }
                }
            } else {
                _plebLiveClear();
        ...
        .catch(function() { /* silent fail — worker might not be deployed yet */ });
}

function _plebLivePlay(videoId, title) {
    _currentVideoId = videoId;
    _np.videoId = videoId;
    _np.videoTitle = '🔴 LIVE: ' + (title || 'Pleb Underground');
    try { _updateNowPlaying(); } catch(e) {}
    // Show LIVE badge
    _showPlebLiveBadge(true);
...
function _plebLiveClear() {
    if (!_plebLiveActive) return;
    _plebLiveActive = false;
    _plebLiveOverride = null;
    _showPlebLiveBadge(false);
    // Resume normal scheduled playback if still on Channel 0
    if (_currentStation === 'pleb-underground') {
        var station = STATIONS.find(function(s) { return s.id === 'pleb-underground'; });
        if (station) {
            var state = getPlaybackState(station);
            if (state && state.video) {
                _currentVideoId = state.video.id;
                _np.videoId = state.video.id;
                _np.videoTitle = state.video.title;
                try { _updateNowPlaying(); } catch(e) {}
                if (_ytPlayer && _ytPlayer.loadVideoById) {
                    try { _ytPlayer.loadVideoById(state.video.id, Math.floor(state.offset)); _ytPlayer.playVideo(); } catch(e) {}
                }
            }
        }
    }
...
function _showPlebLiveBadge(show) {
    var existing = document.getElementById('pleb-live-badge');
    if (!show) { if (existing) existing.remove(); return; }
    if (existing) return; // Already showing
    var badge = document.createElement('div');
    badge.id = 'pleb-live-badge';
    badge.style.cssText = 'position:fixed;top:60px;right:16px;z-index:9999;background:linear-gradient(135deg,#ff0000,#cc0000);color:#fff;padding:6px 14px;border-radius:20px;font-size:0.8rem;font-weight:800;letter-spacing:1px;animation:plebLivePulse 1.5s infinite;box-shadow:0 2px 12px rgba(255,0,0,0.4);cursor:pointer;';
    badge.innerHTML = '🔴 LIVE NOW';
    badge.onclick = function() {
        if (_currentStation !== 'pleb-underground') {
            window.switchStation('pleb-underground');
        }
    };
    document.body.appendChild(badge);
    // Add pulse animation if not already present
    if (!document.getElementById('pleb-live-css')) {
        var style = document.createElement('style');
        style.id = 'pleb-live-css';
        style.textContent = '@keyframes plebLivePulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:0.85;transform:scale(1.05)}}';
...
function _advanceToNextVideo() {
    ...
    // Don't advance when live override is playing on Channel 0
    if (_plebLiveActive && _currentStation === 'pleb-underground') return;
...
function checkDrift() {
    ...
    // Skip drift correction during live override
    if (_plebLiveActive && _currentStation === 'pleb-underground') return;
    var station = STATIONS.find(function(s) { return s.id === _currentStation; });
    if (!station) return;
    var state = getPlaybackState(station);
...
window.tctvDirectChannel = function(val) {
    ...
    if (isNaN(num) || num < 0 || num >= STATIONS.length) return;
    window.switchStation(STATIONS[num].id);
...
var _CHANNEL_FEATURED = {
    'pleb-underground': [
        'Pleb Underground', 'Daily Live Shows', 'Bitcoin Only',
        'Pleb Discussions', 'Weekly Roundups', 'Bitcoin News'
    ],
    'art-philosophy': [
    ...
    'mining': [
        'Jason Lowery', 'Softwar Thesis', 'Power Projection',
        ...
        body += '<div onclick="tctvCloseGuide();window.switchStation(\'' + s.id + '\')" style="padding:12px 14px;margin:4px;border-radius:10px;background:' + (isActive ? 'rgba(247,147,26,0.18)' : '#161616') + ';border:1px solid ' + (isActive ? '#f7931a' : '#222') + ';cursor:pointer;transition:0.15s;" onmouseover="this.style.background=\'rgba(247,147,26,0.10)\';this.style.borderColor=\'#f7931a\';" onmouseout="this.style.background=\'' + (isActive ? 'rgba(247,147,26,0.18)' : '#161616') + '\';this.style.borderColor=\'' + (isActive ? '#f7931a' : '#222') + '\';">' +
            '<div style="display:flex;align-items:center;gap:10px;">' +
                ...
                '<div style="flex:1;min-width:0;">' +
                    '<div style="font-size:0.95rem;font-weight:800;color:#fff;"><span style="color:#f7931a;font-family:Courier New,monospace;font-weight:700;">CH ' + i + '</span> · ' + s.name + (isActive ? ' <span style="font-size:0.6rem;color:#22c55e;font-weight:700;margin-left:4px;">● NOW</span>' : '') + '</div>' +
...
function _renderNP() {
    ...
    if (chEl) chEl.textContent = 'CH. ' + idx + ' · ' + (station.emoji ? station.emoji + ' ' : '') + station.name;
...
function updateTimeline() {
    if (!_currentStation) return;
    // Skip timeline sync when live override is active on Channel 0
    if (_plebLiveActive && _currentStation === 'pleb-underground') return;
    var nowMs = Date.now();
    var station = STATIONS.find(function(s) { return s.id === _currentStation; });
    if (!station) return;
...
function _renderEPG() {
    ...
    STATIONS.forEach(function(s, idx) {
        var isActive = s.id === _currentStation;
        html += '<div onclick="switchStation(\'' + s.id + '\')" data-station-id="' + s.id + '" style="height:64px;display:flex;align-items:center;gap:4px;padding:0 6px;cursor:pointer;border-bottom:1px solid #1a1a1a;background:' + (isActive ? 'rgba(247,147,26,0.12)' : 'transparent') + ';box-sizing:border-box;">';
        html += '<span style="font-size:0.65rem;font-weight:800;color:' + (isActive ? '#f7931a' : '#666') + ';min-width:14px;text-align:right;flex-shrink:0;">' + idx + '</span>';
        html += '<span style="font-size:1rem;flex-shrink:0;">' + s.emoji + '</span>';
        html += '<div style="min-width:0;display:flex;flex-direction:column;justify-content:center;">';
        html += '<div data-ch-name style="font-size:0.65rem;font-weight:700;color:' + (isActive ? '#f7931a' : '#ccc') + ';line-height:1.3;">' + s.name + '</div>';
        html += '<span id="tctv-viewers-' + s.id + '" style="font-size:0.5rem;color:#22c55e;font-weight:600;line-height:1.1;"></span>';
        html += '</div></div>';
    });
    html += '</div>';
    ...
    STATIONS.forEach(function(s) {
        html += '<div onclick="switchStation(\'' + s.id + '\')" style="height:64px;position:relative;border-bottom:1px solid #1a1a1a;">';
...
window.renderTimechainTV = function() {
    ...
    // Sponsor sidebar removed — sponsor/placeholder now overlays the video player (tctv-sponsor-overlay)
    ...
    html += '<div style="flex:1 1 auto;min-width:0;width:100%;max-width:1100px;" class="tctv-video-wrap">' +
            ...
            '</div>' +
            (_tctvSponsor ?
                '<a id="tctv-sponsor-overlay" href="' + _tctvSponsor.url + '" target="_blank" rel="noopener sponsored" style="position:absolute;bottom:14px;left:14px;z-index:5;display:flex;align-items:center;gap:8px;padding:8px 12px;background:rgba(0,0,0,0.75);border:1px solid rgba(247,147,26,0.25);border-radius:10px;text-decoration:none;backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);transition:opacity 0.3s;max-width:220px;">' +
                    (_tctvSponsor.logo ? '<img src="' + _tctvSponsor.logo + '" alt="' + _tctvSponsor.name + '" style="width:32px;height:32px;border-radius:6px;object-fit:contain;flex-shrink:0;">' : '') +
                    '<div style="min-width:0;"><div style="font-size:0.5rem;color:#888;text-transform:uppercase;letter-spacing:1px;font-weight:700;">Sponsor</div><div style="font-size:0.7rem;color:#eee;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + _tctvSponsor.name + '</div></div>' +
                '</a>'
            :
                '<a id="tctv-sponsor-overlay" href="mailto:info.603btc@gmail.com" style="position:absolute;bottom:14px;left:14px;z-index:5;display:flex;align-items:center;gap:8px;padding:8px 12px;background:rgba(0,0,0,0.75);border:1px solid rgba(247,147,26,0.15);border-radius:10px;text-decoration:none;backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);transition:opacity 0.3s;max-width:220px;">' +
                    '<div style="font-size:1.1rem;flex-shrink:0;">\ud83c\udf1f</div>' +
                    '<div style="min-width:0;"><div style="font-size:0.55rem;color:#aaa;font-weight:700;">Your Brand Here</div><div style="font-size:0.5rem;color:#888;">Reach Bitcoiners 24/7</div></div>' +
                    '<div style="font-size:0.5rem;color:#f7931a;font-weight:700;flex-shrink:0;">Inquire</div>' +
                '</a>'
            ) +
    ...
    // Start Pleb Underground live polling (checks every 60s for live broadcasts)
    _startPlebLivePolling();
...
window.switchStation = function(stationId, forceUpdate) {
    ...
    // Pleb Underground live override: if switching to CH0 and a live stream is active, use that
    var _useLiveOverride = (stationId === 'pleb-underground' && _plebLiveActive && _plebLiveOverride);
    ...
    if (_useLiveOverride) {
        // Live stream override for Channel 0
        _plebLivePlay(_plebLiveOverride.videoId, _plebLiveOverride.title);
